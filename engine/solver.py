...
class Trainer(object):
        ...
        with tqdm(initial=step, total=self.train_num_steps) as pbar:
            while step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    input = data[:, :, 0].unsqueeze(-1)
                    conditional_features = data[:, :, 1:]

                    loss = self.model(input, conditional_features = conditional_features, target=input)
    ...
    def sample(self, shape=None, dataloader=None):
        if self.logger is not None:
            tic = time.time()
        ...
        for _, x in enumerate(dataloader):
            x = x.to(self.device)
            conditional_features = x[:, :, 1:]
            sample = self.ema.ema_model.generate_samples(batch_size=x.shape[0], cond_input=conditional_features)
